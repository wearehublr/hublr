...
describe("createResilientFetch", () => {
  ...
  it("retries once on a 504 Gateway Timeout for a GET request", async () => {
    const mockFetch = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse(504, { message: "Gateway Timeout" }))
      .mockResolvedValueOnce(jsonResponse(200, { ok: true }));
    vi.stubGlobal("fetch", mockFetch);
  ...
  it("does not retry a 504 for a non-GET request", async () => {
    const mockFetch = vi.fn().mockResolvedValue(jsonResponse(504, { message: "Gateway Timeout" }));
    vi.stubGlobal("fetch", mockFetch);

    const resilientFetch = createResilientFetch();
    const response = await resilientFetch("https://example.com", { method: "POST" });
